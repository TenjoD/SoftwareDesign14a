        ...
        if r.status_code == 200:
            for c in r.json():
                tree.insert("", "end", values=(
                    c["id"], c["nombre"], c["descripcion"], c["precio"], c["categoria"]
                ))
...
def eliminar_producto():
    ...
    if messagebox.askyesno("Eliminar", "¿Seguro que desea eliminar este producto?"):
        try:
            r = requests.delete(API + ENDPOINTS["delete"].replace("{id}", str(id_prod)))
            if r.status_code == 200:
                recargar_datos()
...
def dialogo_producto(titulo, datos=None):
    ...
    if datos:
        id_prod, nombre_val, descripcion_val, precio_val, categoria_val = datos
    ...
    def guardar():
        ...
        try:
            if datos:
                r = requests.put(API + ENDPOINTS["update"].replace("{id}", str(id_prod)), json=payload)
            else:
...
# Tabla
cols = ("ID", "Nombre", "Descripción", "Precio", "Categoría")
